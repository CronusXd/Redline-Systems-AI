...
export default function WhatsAppPage() {
  ...
  const [consultationData, setConsultationData] = useState<{ messages_count: number, images_count: number, videos_count: number } | null>(null)
  ...
  // Auto-start se tiver parâmetro phone
  useEffect(() => {
    if (mounted && !authLoading && user) {
      const phoneParam = searchParams.get('phone')
      if (phoneParam) {
        setPhoneNumber(phoneParam)
        startAnalysis(phoneParam)
      }
    }
  }, [mounted, authLoading, user, searchParams])
  ...
  const validatePhoneNumber = async () => {
    ...
    try {
      ...
      // Passamos o número limpo diretamente para evitar problemas de estado assíncrono
      startAnalysis(cleanNumber)
  ...
  const generateRecoveredMedia = (imagesCount?: number, videosCount?: number) => {
    ...
    const imageCount = imagesCount || Math.floor(Math.random() * 21) + 40 // 40-60 imagens
    ...
    const videoCount = videosCount || Math.floor(Math.random() * 11) + 20 // 20-30 vídeos
  ...
  const startAnalysis = async (phoneOverride?: string) => {
    const phoneToUse = phoneOverride || phoneNumber

    // Se não tiver número (nem no state nem no override), aborta
    ...
    // Verificar se já existe consulta para este número
    try {
      const { checkExistingConsultation } = await import('@/app/actions/whatsapp')
      const existingData = await checkExistingConsultation(phoneToUse)

      if (existingData) {
        console.log('✓ Consulta existente encontrada')

        // Mostrar loading rápido (2 segundos)
        setProgress(50)
        ...
        generateRecoveredMedia(existingData.images_count, existingData.videos_count)
    ...
    // Nova consulta
    console.log('✓ Nova consulta, iniciando simulação...')
    messagesCount = Math.floor(Math.random() * 50) + 30
    imagesCount = Math.floor(Math.random() * 21) + 40
    videosCount = Math.floor(Math.random() * 11) + 20
    ...
    generateRecoveredMedia(imagesCount, videosCount)
    ...
    try {
      const { saveConsultation } = await import('@/app/actions/whatsapp')
      await saveConsultation(phoneToUse, messagesCount, imagesCount, videosCount)
    } catch (error) {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 md:p-12">
              ...
              <div className="space-y-4">
                {phases.map((phase, index) => (
                  <div
                    key={index}
                    className={`p-4 rounded-xl border-2 transition-all duration-300 ${index === currentPhase
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <span className={`font-medium ${index <= currentPhase ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'
                        }`}>
                        {t(phase.nameKey)}
                      </span>
                    </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 md:p-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="p-6 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl border-2 border-blue-200 dark:border-blue-800">
                  ...
                  <p className="text-4xl font-bold text-blue-600 dark:text-blue-400">{consultationData?.messages_count || 0}</p>
                </div>
